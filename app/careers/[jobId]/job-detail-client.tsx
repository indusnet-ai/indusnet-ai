...
import { 
  ...
  Upload, CheckCircle, AlertCircle, Sparkles, Building, Phone, Cpu
} from "lucide-react";
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        alert("File size exceeds 10MB limit. Please upload a smaller PDF/DOCX file.");
        return;
      }
      setResumeFile(file);
    }
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">Loading job specifications...</span>
        </div>
      </div>
    );
  }
  ...
  if (!job) {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-muted-foreground text-xs mb-6 max-w-sm">This position may have been closed or is temporarily in draft mode.</p>
        <Button asChild variant="outline" className="border-border/40 hover:bg-muted rounded-full text-xs">
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-6 md:px-12 py-12 grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <section className="lg:col-span-6 flex flex-col gap-6 text-left" aria-labelledby="job-title-heading">
          <Link href="/careers" className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1.5 transition-colors mb-2">
          ...
          <div>
            ...
            <h1 id="job-title-heading" className="text-3xl md:text-4xl font-extrabold text-foreground leading-tight mb-4">
              {job.title}
            </h1>
          ...
          <div className="flex flex-col gap-3">
            <h2 className="text-base font-bold text-foreground uppercase tracking-wider">Role Overview</h2>
            <p className="text-muted-foreground text-xs md:text-sm leading-relaxed whitespace-pre-wrap">
              {job.description}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 mt-4">
            <h2 className="text-base font-bold text-foreground uppercase tracking-wider">Key Requirements</h2>
            <div className="text-muted-foreground text-xs md:text-sm leading-relaxed whitespace-pre-wrap bg-muted border border-border p-4 rounded-xl">
              {job.requirements}
            </div>
          </div>
        </section>
        ...
        <section className="lg:col-span-6" aria-labelledby="form-heading">
          {success ? (
            <Card className="bg-emerald-500/10 border border-emerald-500/20 p-8 text-center rounded-2xl flex flex-col items-center gap-4 shadow-2xl backdrop-blur-xl">
              <CheckCircle className="w-16 h-16 text-emerald-400 animate-bounce" />
              <CardTitle className="text-xl font-bold text-foreground">Application Received!</CardTitle>
              <p className="text-muted-foreground text-xs leading-relaxed max-w-sm">
                Thank you for applying. Our AI recruitment copilot is currently analyzing your resume. A confirmation email has been dispatched to <strong className="text-foreground">{email}</strong>.
              </p>
            ...
            <Card className="glassmorphism-card border-none p-6 md:p-8">
              <CardHeader className="p-0 pb-6 text-left">
                <CardTitle id="form-heading" className="text-lg font-bold text-foreground flex items-center gap-2">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5 text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="fullname" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Full Name</label>
                    <Input id="fullname" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required className="bg-muted border-border text-xs text-foreground" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="email" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Email Address</label>
                    <Input id="email" type="email" placeholder="john@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="bg-muted border-border text-xs text-foreground" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input id="phone" type="tel" placeholder="+65-9876-5432" value={phone} onChange={(e) => setPhone(e.target.value)} required className="bg-muted border-border text-xs text-foreground" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="experience" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Years of Experience</label>
                    <Input id="experience" type="number" step="0.1" min="0" placeholder="e.g. 4.5" value={experienceYears} onChange={(e) => setExperienceYears(e.target.value)} required className="bg-muted border-border text-xs text-foreground" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-border/5 pt-4">
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="company" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Current Company</label>
                    <Input id="company" placeholder="e.g. Acme Corp" value={currentCompany} onChange={(e) => setCurrentCompany(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="designation" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Current Designation</label>
                    <Input id="designation" placeholder="e.g. Senior Software Engineer" value={currentDesignation} onChange={(e) => setCurrentDesignation(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="salary" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Expected Salary</label>
                    <Input id="salary" placeholder="e.g. SGD 8,000/mo" value={expectedSalary} onChange={(e) => setExpectedSalary(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="notice" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Notice Period</label>
                    <Input id="notice" placeholder="e.g. 1 Month" value={noticePeriod} onChange={(e) => setNoticePeriod(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-border/5 pt-4">
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="linkedin" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">LinkedIn Profile URL</label>
                    <Input id="linkedin" type="url" placeholder="https://linkedin.com/in/username" value={linkedinUrl} onChange={(e) => setLinkedinUrl(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="portfolio" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Portfolio / GitHub URL</label>
                    <Input id="portfolio" type="url" placeholder="https://github.com/username" value={portfolioUrl} onChange={(e) => setPortfolioUrl(e.target.value)} className="bg-muted border-border text-xs text-foreground" />
                  </div>
                </div>

                {/* Section 4: File Upload */}
                <div className="flex flex-col gap-1.5 border-t border-border/5 pt-4">
                  <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Resume Attachment (PDF or DOCX)</span>
                  <div className="border border-dashed border-border/40 hover:border-primary/50 transition-all rounded-xl p-6 text-center cursor-pointer relative bg-muted/30 focus-within:ring-1 focus-within:ring-primary">
                    <input
                      type="file"
                      id="resume-upload"
                      accept=".pdf,.docx"
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Max file size: 10MB. Formats: PDF or DOCX only.
                    </p>
                  </div>
                </div>
